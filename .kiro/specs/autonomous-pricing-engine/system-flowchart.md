...
flowchart TD
    ...
    MS -->|Detects price change more than 5%| MS1{Data Quality Check}
    MS1 -->|Pass| MS2[Enrich with context]
    MS1 -->|Fail - more than 80% deviation| MS3[Flag for manual review]
    ...
    GR1 -->|Velocity limit OK?| GR2{Confidence more than 0.5?}
    GR2 -->|All pass| EXEC[Execution Agent]
    GR2 -->|Confidence less than 0.5| HUMAN1[Escalate to Human]
    ...
    MON1 -->|Deviation more than 20%| MON2[Detect anomaly]
    ...
    CORR3 -->|Less than 3 attempts| SUP3[Supervisor Agent]
    CORR3 -->|3 or more attempts| HUMAN3[Escalate to Human]
    SUP3 -->|Re-validate| GR
...
flowchart TD
    ...
    GR1 -->|margin 25% or more| GR2{Maximum Discount Check}
    GR1 -->|margin less than 25%| REJECT1[Reject: Margin too low]
    
    GR2 -->|discount 20% or less| GR3{Velocity Limit Check}
    GR2 -->|discount more than 20%| REJECT2[Reject: Discount too high]
    
    GR3 -->|changes less than 3/day| GR4{Price Change Magnitude}
    GR3 -->|changes 3 or more/day| QUEUE[Queue for later]
    
    GR4 -->|change less than 30%| GR5{Confidence Score}
    GR4 -->|change 30% or more| HUMAN1[Human approval required]
    
    GR5 -->|confidence 0.8 or more| APPROVE1[Approve: Execute autonomously]
    GR5 -->|confidence 0.5 to 0.8| APPROVE2[Approve: Execute with<br/>enhanced monitoring]
    GR5 -->|confidence less than 0.5| HUMAN2[Human approval required]
    
    GR5 -->|Also check| LOSS{Daily Loss Limit}
    LOSS -->|loss less than Rs 10,000| CONTINUE[Continue]
    LOSS -->|loss Rs 10,000 or more| PAUSE[Pause autonomous pricing]
...
flowchart TD
    ...
    MON1 --> MON2[Collect actual data:<br/>Sales volume, Revenue, Margin]
    MON2 --> MON3[Compare vs predicted:<br/>Expected: +5% revenue<br/>Actual: -12% revenue]
    
    MON3 --> CHECK{Performance Check}
    CHECK -->|Deviation less than 20%| SUCCESS([Decision Successful])
    CHECK -->|Deviation 20% or more| ANOM[Anomaly Detected]
    ...
    CORR4 -->|Competitor Response| RC1[Competitor dropped further<br/>to Rs 850 - we missed it]
    ...
    CORR5 --> CORR6[AI generates reasoning:<br/>Reduce to Rs 900 to match<br/>competitor and stop decline]
...
```mermaid
flowchart TD
    START([Demo Start]) --> SETUP[Setup: Product P123<br/>Current price: Rs 1000<br/>Margin: 40%]
    
    SETUP --> SCENE1[Scene 1: Market Change]
    SCENE1 --> S1A[Competitor C456 drops price<br/>Rs 1000 to Rs 900 - 10% drop]
    ...
    S2A --> S2B[Bedrock generates reasoning:<br/>Competitor dropped 10%<br/>Recommend Rs 950 - 37% margin<br/>Expected +5% revenue]
    S2B --> S2C[Show: Dashboard with AI reasoning<br/>Confidence: 75%]
    
    S2C --> SCENE3[Scene 3: Guardrails]
    SCENE3 --> S3A[Supervisor validates:<br/>Margin 37% more than 25% min<br/>Discount 5% less than 20% max<br/>Confidence 75% more than 50%]
    S3A --> S3B[Show: Guardrail checks passing]
    
    S3B --> SCENE4[Scene 4: Execution]
    SCENE4 --> S4A[Execution Agent updates:<br/>Price: Rs 950 + 18% GST = Rs 1121]
    ...
    SCENE6 --> S6A[Correction Agent investigates:<br/>Competitor dropped again to Rs 850]
    S6A --> S6B[AI root cause analysis:<br/>Missed second price drop<br/>Recommend Rs 900 to recover]
    S6B --> S6C[Show: Corrective decision<br/>with AI reasoning]
    S6C --> S6D[Re-execute through Supervisor]
    S6D --> S6E[Price updated to Rs 900<br/>Linked to original decision]
    
    S6E --> SCENE7[Scene 7: Results]
    SCENE7 --> S7A[Show complete decision chain:<br/>Original to Underperformance to Correction]
